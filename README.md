...
查资料、读网页、进已经登录的网站办事，往往会用到不同的联网方式。这个 Agent Skill 补上的是：**联网策略 + CDP 浏览器操作 + 站点经验积累**。Agent 根据任务选择搜索、抓取或浏览器，复用你日常浏览器的登录态，把走通的网站操作经验留下来。兼容支持 SKILL.md 的 Agent（Claude Code、Cursor、Gemini CLI、Codex CLI 等）。

## 可以直接交给它的网页任务

- **查最新资料、比较多个来源**：“同时调研这 5 个产品的官网，给我对比摘要。”也可以指定一个话题，查近期进展并读原页面。
- **读动态页面或登录后的内容**：“读一下这个页面，把我需要的信息找出来。”需要页面渲染时接入浏览器，不停在搜索摘要上。
- **找回记不住地址的页面**：“上周看过的那个内部平台，帮我找一下。”从浏览器历史和书签里按关键词、时间定位。
- **处理网站里的操作**：“去小红书找这个账号”“帮我在创作者平台发一篇图文。”按你的请求搜索、填写或上传文件。
- **看网页里的图片和视频**：“截一下视频这个时间点，看看画面里是什么。”提取媒体地址或截帧，把页面里的视觉内容也用起来。
...
安装并完成前置配置后，直接把上面的网页任务交给 Agent 即可。
