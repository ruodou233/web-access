...
为避免普通网页任务顺带修改 skill，旧写法收到 `400` + 包含本文档路径的错误信息后，按下方对照表改写当前调用；仅在用户明确要求维护本 skill 时，才修正源站点经验文件。
...
3. **按维护范围更新源文件**：仅在用户明确要求维护本 skill 时，修正 `references/site-patterns/*.md` 中已核实的旧写法；普通网页任务只改当前调用。
